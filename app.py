...
with st.sidebar:
    ...
    if role in ("Admin","Manager","User"):
        if st.button("➕ إضافة بلاغ جديد", key="nav_add"):
            st.session_state["page"]="add_case"; st.session_state["edit_case_id"]=None; st.rerun()
    if st.button("🔍 البحث وعرض البلاغات", key="nav_search"):
        st.session_state["page"]="search"; st.session_state["edit_case_id"]=None; st.rerun()
    if role in ("Admin","Manager","User"):
        if st.button("📤 استيراد من Excel", key="nav_import"):
            st.session_state["page"]="import"; st.rerun()
    if role in ("Admin","Manager"):
        st.markdown("---")
        st.markdown("<div style='font-size:0.75rem;opacity:0.7'>⚙️ الإدارة</div>", unsafe_allow_html=True)
        if st.button("👥 إدارة المستخدمين", key="nav_users"):
            st.session_state["page"]="manage_users"; st.rerun()
        if st.button("📚 إدارة المراجع RESORS", key="nav_resources"):
            st.session_state["page"]="manage_resources"; st.rerun()
